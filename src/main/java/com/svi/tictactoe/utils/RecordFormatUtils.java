...
/** Converts the legacy flat-file records to and from CSV. */
public final class RecordFormatUtils {
    ...
    public static String gameToCsv(GameRecordDTO record) {
        if (record.getPlayerName() != null && !record.getPlayerName().isEmpty()) {
            return String.format("%s,%s,%s,%s,%s,%s",
                    record.getGameId(),
                    record.getPlayerId(),
                    escapeCsv(record.getPlayerName()),
                    record.getSymbol(),
                    record.getLocation(),
                    record.getDateSaved());
        }
    ...
    public static GameRecordDTO gameFromCsv(String line) {
        ...
        if (fields.length == 5) {
            return new GameRecordDTO(fields[0], fields[1], fields[2], fields[3], fields[4]);
        }
        if (fields.length == 6) {
